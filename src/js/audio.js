...
class AudioPlayer {
    ...
    static init(selector) {
        ...
        audio.forEach((audioEl) => {
            ...
            playerEl.classList.add('audio-player-wrapper');
            playerEl.innerHTML = `<div class="audio-player" id="${id}">${AudioPlayer.getControls()}</div>`;

            audioEl.parentNode.insertBefore(playerEl, audioEl);

            AudioPlayer.players.push(playerEl);
            AudioPlayer.attachEvents(playerEl, audioEl);
        });
